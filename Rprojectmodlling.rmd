...
```{r}
#  one can get specific details by using various specific functions sich as mean,min,max,median,sd,quartile etc except mode which one has to create a function for
...
advert$Clicked_on_Ad = factor(advert$Clicked_on_Ad , levels = c(0,1))
```
...
table(p, advert$Clicked_on_Ad )
```
```{r}
mean(advert$Clicked_on_Ad  == p)
```
...
3. Decision tree modelling yield 100 accuracy
